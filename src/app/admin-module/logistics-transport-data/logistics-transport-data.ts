...
@Component({
  selector: 'app-logistics-transport-data',
  imports: [CommonModule,FormsModule],
  templateUrl: './logistics-transport-data.html',
  styleUrl: './logistics-transport-data.css'
})
export class LogisticsTransportData {
  @ViewChild('fileInput') fileInput!: ElementRef;
    ...
    onFileChange(event: any): void {
      ...
      this.http.post<logisticstransportData[]>(`${environment.apiUrl}/api/LogisticsTransport/upload-logisticstransport`, formData).subscribe({
        next: (data) => {
    ...
    clearData(): void {
      if (confirm('Are you sure you want to clear all data from the database?')) {
        this.http.delete(`${environment.apiUrl}/api/LogisticsTransport/delete-all`).subscribe({
          next: () => {
            this.tableData = [];
