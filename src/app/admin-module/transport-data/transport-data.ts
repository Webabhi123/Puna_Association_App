...
@Component({
  ...
  templateUrl: './transport-data.html',
  styleUrl: './transport-data.css'
})
export class TransportData {
  ...
  onFileChange(event: any): void {
    ...
    this.http.post<transportData[]>(`${environment.apiUrl}/api/Excel/upload-excel`, formData).subscribe({
      next: (data) => {
  ...
  clearData(): void {
    if (confirm('Are you sure you want to clear all data from the database?')) {
      this.http.delete(`${environment.apiUrl}/api/Excel/delete-all`).subscribe({
        next: () => {
          this.tableData = [];
